...
export function shouldBehaveCorrectFundingDefaultCase(): void {
  describe('funding: default case', () => {
    ...
    it('user1 tries to call updateAccountItem without permission', async function () {
      await expect(
        this.user1WEB3ProRata.updateAccountItem(this.user1Address, {
    ...
    it('user1 tries to call updateAccountItems without permission', async function () {
      await expect(
        this.user1WEB3ProRata.updateAccountItems(
          [this.user1Address],
          [
            {
              manual: seedData.manual,
    ...
    it('owner2 tries to call updateAccountItem without account item existence ', async function () {
      await expect(
        this.owner2WEB3ProRata.updateAccountItem(this.user1Address, {
          manual: seedData.manual,
          baseAllocation: seedData.baseAllocation1,
          baseRefund: seedData.baseRefund1,
          boostRefund: seedData.boostRefund1,
        }),
      ).revertedWithCustomError(this.owner2WEB3ProRata, customError.nonExistAccountItem);
    });

    it('owner2 tries to call updateAccountItems without account item existence', async function () {
      await expect(
        this.owner2WEB3ProRata.updateAccountItems(
          [this.user1Address],
          [
            {
              manual: seedData.manual,
    ...
    it('owner2 tries to call updateAccountItems without correct array length', async function () {
      await expect(
        this.owner2WEB3ProRata.updateAccountItems([this.user1Address], []),
      ).revertedWithCustomError(this.owner2WEB3ProRata, customError.arrayLengthsNotEqual);
    });
      ...
      describe('user1 and user2 have tokens and approved contract to use these tokens', () => {
        ...
        describe('user1 deposited base tokens', () => {
          ...
          it(INITIAL_POSITIVE_CHECK_TEST_TITLE, async function () {
            ...
            expect(await this.owner2WEB3ProRata.getAccountBaseRefund(this.user1Address)).eq(
              seedData.deposit1,
            );
          ...
          it('owner2 called updateAccountItem (check event)', async function () {
            const initialAccountCount = await this.owner2WEB3ProRata.getAccountCount();
            const initialUpdateTimestamp = await time.latest();
            expect(await this.owner2WEB3ProRata.getAccountItemUpdateLogs()).eql([]);
            ...
            const accountInfo1 = await this.user1WEB3ProRata.fetchAccountInfo(this.user1Address);
            ...
            expect(await this.owner2WEB3ProRata.getAccountCount()).eq(initialAccountCount);

            const updateLogs = await this.owner2WEB3ProRata.getAccountItemUpdateLogs();
          ...
          describe(`set time after close date when goal wasn't reached`, () => {
            ...
            it(INITIAL_POSITIVE_CHECK_TEST_TITLE, async function () {
              ...
              expect(await this.owner2WEB3ProRata.getAccountBaseRefund(this.user2Address)).eq(
                seedData.zero,
              );
          ...
          describe('user2 deposited base tokens', () => {
            ...
            describe('set time after close date when goal was reached', () => {
              ...
              it(INITIAL_POSITIVE_CHECK_TEST_TITLE, async function () {
                expect(
                  await this.owner2WEB3ProRata.getAccountBaseRefund(this.user1Address),
                ).closeTo(
                ...
                expect(
                  await this.owner2WEB3ProRata.getAccountBaseRefund(this.user2Address),
                ).closeTo(
                  calculateAccountRefund(
                    contractConfig.baseGoal,
                    seedData.deposit2,
