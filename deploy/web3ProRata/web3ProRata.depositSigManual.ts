...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment): Promise<void> => {
  await callWithTimerHre(async () => {
    ...
    const { depositVerifier, user1Address, user1BaseToken, user1WEB3ProRata, web3ProRataFactory } =
      context;

    const { decimals, tokenName } = await getBaseTokenInfo(await getUsers(), user1WEB3ProRata);
    ...
    if (CHECK_REQUIRED) {
      if (body.contractAddress.toLowerCase() !== web3ProRataAddress.toLowerCase()) {
      ...
      const nonce = await user1WEB3ProRata.getAccountDepositNonce(body.account);
    ...
    if (SIMULATE_FRONT) {
      ...
      //   contractAddress: web3ProRataAddress,
    ...
    if (params.baseAmount > currentAllowance) {
      ...
      console.log(
        `${toNumberDecimals(askAllowance, decimals)} WEB3 was approved to ${web3ProRataAddress}`,
      );
    }

    // await waitTx(user1BaseToken.approve(web3ProRataAddress, 0n, TX_OVERRIDES), 'approve');
...
func.tags = [`${WEB3_P_PRO_RATA_NAME}:deposit-sig-manual`];
